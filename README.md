...
### 1-4. 사람 승인 게이트 — AI 가 쓴 답장을 텔레그램에서 승인해야 나간다

`문의 웹훅` → `AI 답장 초안` → **`사람 승인 대기`** → (승인) `고객에게 발송` / (거절) 종료 / (만료) 알림

회사가 AI 자동화에서 가장 꺼리는 건 "AI 가 고객에게 멋대로 보내는 것"이다. 이 노드는 그 반대를 강제한다. 워크플로가 여기서 **멈추고**, 사람이 텔레그램 버튼(✅ 승인 · ✏️ 수정 · ❌ 거절)을 누른 뒤에만 아래로 흐른다.

- **멈춤** — 노드가 `{ __wait }` 를 내면 엔진이 그 노드를 `waiting` 으로 두고, 그 아래 노드는 전부 건너뛴다(입력이 여럿인 노드가 반쪽 입력으로 돌지 않게). 승인과 무관한 가지는 계속 흐른다. **메시지는 실행이 끝난 뒤에** 보낸다 — 그때 끝난 노드 전부의 출력과 노드·엣지를 `approvals.json` 에 스냅샷으로 남기므로, 실행이 끝나기 전에 버튼을 누를 수 없고 재개 때 어떤 노드도 두 번 돌지 않는다.
- **재개** — 결정이 오면 스냅샷을 `seed` 로 주입해 같은 워크플로를 다시 실행한다. 이미 끝난 노드는 재실행되지 않고, `approved`·`rejected`·`expired` 세 포트 중 하나로만 흐른다. 결정은 **멱등**이라 버튼을 두 번 눌러도 한 번만 재개된다.
- **서버가 재시작돼도** 대기 중인 건은 파일에 남아 있어, 버튼을 늦게 눌러도 이어진다. 리마인드(기본 60분)는 한 번만, 만료(기본 24시간)되면 `expired` 포트로 흐른다.
- **텔레그램은 롱폴링**(`getUpdates`)이라 공개 URL 이 없어도 된다 — 노트북에서 돌려도 버튼이 동작한다. 요청을 보낸 채팅에서 온 결정만 받고, `TELEGRAM_CHAT_ID` 로 허용 채팅을 제한한다.
- ✏️ 수정을 누르면 안내 메시지에 대한 **답장만** 수정본으로 받아 `approval.text` 로 넣는다(서버 재시작·여러 건이 겹쳐도 섞이지 않는다).
- 승인은 됐는데 이후 실행이 실패하면 메시지를 "⚠️ 승인됐지만 실행 실패: 원인" 으로 바꾸고 🔁 다시 시도 버튼을 남긴다. 재개 도중 서버가 죽은 건은 기동 시 찾아 같은 안내를 보낸다.
- 캔버스의 로컬 실행(서버 없이)에서는 승인 노드가 통과하지 않고 대기로만 표시된다 — HTTP·코드 노드는 브라우저에서도 진짜로 돌기 때문.
- 채널이 설정돼 있지 않거나 메시지 전송이 실패하면 **조용히 승인된 것처럼 통과하지 않고** 노드 오류가 난다.
- `/api/approvals` 로 대기 목록을 보고 API 로도 결정할 수 있다(API 키 인증). Slack 등 다른 채널은 어댑터 하나(`setApprovalAdapter`)로 붙는다.

구현: `src/engine/executor.js`(waiting), `server/approvals.js`(스냅샷·재개·멱등·만료), `server/telegram.js`(버튼·롱폴링). 테스트: `tests/engine/waiting.test.js`, `tests/server/approvals.test.js`, `tests/server/telegram.test.js`.
...
| **규모** | 노드 56종 (트리거 4·동작 7·흐름 제어 6·배열 5·연동 10·AI 4·영상 10·수익화 8·출력 2) · 프론트+서버 약 8,800줄 |
| **실사용** | 쇼츠 자동 제작 파이프라인(스케줄 → 대본 → TTS → Remotion 렌더 → YouTube API 업로드)을 실제 채널에 연결해 업로드까지 동작을 확인 — [Conduit으로 만든 것](#conduit으로-만든-것) |
| **테스트** | Vitest 222개 — 실행 엔진(실행 순서·분기·배치·병렬·재시도·오류 격리), 표현식, 재시도 정책, 노드 동작, API 인증·라우트 통합(실제 HTTP), 코드 실행 정책, MCP 패키지(stdio 엔드투엔드), 평가 실행기. `npm test` |
